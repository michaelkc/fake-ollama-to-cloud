...
builder.Services.Configure<CloudApiOptions>(builder.Configuration.GetSection(CloudApiOptions.SectionName));
builder.Services.AddHttpClient(CloudApiClient.HttpClientName);
builder.Services.AddSingleton<CloudApiClient>();

var app = builder.Build();

var cloudApiConfig = app.Services.GetRequiredService<Microsoft.Extensions.Options.IOptions<CloudApiOptions>>().Value;
if (string.IsNullOrWhiteSpace(cloudApiConfig.BaseUrl))
{
    app.Logger.LogError("CloudApi:BaseUrl is not configured. Set it in appsettings.json or the CloudApi__BaseUrl environment variable.");
}
if (string.IsNullOrWhiteSpace(cloudApiConfig.ApiKey))
{
    app.Logger.LogWarning("CloudApi:ApiKey is empty. Requests to the upstream API will be sent without an Authorization header.");
}
